...
void PangolinWindowImpl::createDisplayLayout()
{
  ...
  pangolin::View &left_image_show  = pangolin::Display( left_image_view_name_ ).SetBounds( 1 / 3.0f, 2 / 3.0f, 0., 0.98f, 800 / 480. );
  pangolin::View &right_image_show = pangolin::Display( right_image_view_name_ ).SetBounds( 2 / 3.0f, 1.0f, 0.0f, 0.98f, 800 / 480. );
  pangolin::View &plot_show        = pangolin::Display( dis_plot_name_ ).SetBounds( 0.0f, 1.0, 0.70, 1.0 ).AddDisplay( *plotter_angle_ ).AddDisplay( left_image_show ).AddDisplay( right_image_show );
  ...
  gl_texture_left_image_  = std::make_unique<pangolin::GlTexture>( image_width_, image_height_, GL_RGB, false, 0, GL_BGR, GL_UNSIGNED_BYTE );
...
void PangolinWindowImpl::setViewImage( const cv::Mat &left_image, const cv::Mat &right_image )
{
  ...
  cv::resize( left_dist, left_image_, cv::Size( image_width_, image_height_ ) );
  cv::resize( right_dist, right_image_, cv::Size( image_width_, image_height_ ) );
}

bool PangolinWindowImpl::renderViewImage()
{
  assert( left_image_.cols > 0 && left_image_.rows > 0 );
  assert( right_image_.cols > 0 && right_image_.rows > 0 );

  {
    std::unique_lock<std::mutex> lock( update_image_mutex_ );
    gl_texture_left_image_->Upload( left_image_.data, GL_BGR, GL_UNSIGNED_BYTE );
  ...
  pangolin::Display( left_image_view_name_ ).Activate();
  glColor3f( 1.0, 1.0, 1.0 );
  gl_texture_left_image_->RenderToViewportFlipY();
...
{
  std::unique_lock<std::mutex> lock( update_image_mutex_ );
  left_image_ = cv::Mat( image_height_, image_width_, CV_8UC3, cv::Scalar( 255 ) );
  // left_img_：要绘制文本的图像。
  // "NoImg"：要绘制的文本内容。
  ...
  cv::putText( left_image_, "No Image", cv::Point2i( image_width_ / 2 - 60, image_height_ / 2 ), 2, 2, cv::Scalar( 255, 255, 255 ) );
