...
#define TIME_DIFF 0.01
  ...
  bool FrontEnd::PredictIMUState()
  {
    // TODO: implement preintegration
    // 1. When the first StereoFrame(f_1) is grabbed, just return
    if ( this->current_frame_->getLastFramePtr() == nullptr )
    {
      WARN( lk_vio::logger, "No previous frame available for preintegration" );
      return false;
    }
    ...
    if ( this->reference_kf_ != nullptr )
    {
      ...
      const Eigen::Vector3d Gz( 0.0f, 0.0f, -GRAVITY );

      const float t12 = this->preintegrator_from_kf_->dT;

      Eigen::Matrix3d r_w_imu_2 = NormalizeRotation( r_w_imu_1 * this->preintegrator_from_kf_->GetDeltaRotation( reference_kf_->getIMUBias() ) );
      Eigen::Vector3d t_w_imu_2 = t_w_imu_1 + v_w_imu_1 * t12 + 0.5f * Gz * t12 * t12 + r_w_imu_1 * this->preintegrator_from_kf_->GetDeltaPosition( reference_kf_->getIMUBias() );
      Eigen::Vector3d v_w_imu_2 = v_w_imu_1 + Gz * t12 + r_w_imu_1 * this->preintegrator_from_kf_->GetDeltaVelocity( reference_kf_->getIMUBias() );


      this->current_frame_->setIMUPoseAndVelocity( Sophus::SE3d( r_w_imu_2, t_w_imu_2 ), v_w_imu_2 );
      this->current_frame_->imu_bias_     = this->reference_kf_->getIMUBias();
      this->current_frame_->pre_imu_bias_ = this->reference_kf_->getIMUBias();

      return true;
    }
    else if ( this->last_frame_ != nullptr )
    {
      const Eigen::Vector3d t_w_imu_1 = this->last_frame_->getTranslationWorldToCamera();
      const Eigen::Matrix3d r_w_imu_1 = this->last_frame_->getRotationWorldToCamera();
      const Eigen::Vector3d v_w_imu_1 = this->last_frame_->getVelocity();

      const Eigen::Vector3d Gz( 0.0, 0.0, -GRAVITY );

      const float t12 = this->current_frame_->imu_preintegration_lf_->dT;

      Eigen::Matrix3d r_w_imu_2 = NormalizeRotation( r_w_imu_1 * this->current_frame_->imu_preintegration_lf_->GetDeltaRotation( reference_kf_->getIMUBias() ) );
      Eigen::Vector3d t_w_imu_2 = t_w_imu_1 + v_w_imu_1 * t12 + 0.5f * Gz * t12 * t12 + r_w_imu_1 * this->preintegrator_from_kf_->GetDeltaPosition( reference_kf_->getIMUBias() );
      Eigen::Vector3d v_w_imu_2 = v_w_imu_1 + Gz * t12 + r_w_imu_1 * this->current_frame_->imu_preintegration_lf_->GetDeltaVelocity( reference_kf_->getIMUBias() );


      this->current_frame_->setIMUPoseAndVelocity( Sophus::SE3d( r_w_imu_2, t_w_imu_2 ), v_w_imu_2 );
      this->current_frame_->imu_bias_     = this->last_frame_->getIMUBias();
      this->current_frame_->pre_imu_bias_ = this->last_frame_->getIMUBias();
  ...
  bool FrontEnd::BuidInitMap()
  {
    ...
    for ( size_t i = 0, N = current_frame_->features_left_.size(); i < N; i++ )
    {
      ...
      std::vector<Eigen::Vector3d> points{
          left_camera_->pixel2camera( cv_point2f_to_vec2( current_frame_->features_left_[ i ]->kp_position_.pt ) ),
          right_camera_->pixel2camera( cv_point2f_to_vec2( current_frame_->features_right_[ i ]->kp_position_.pt ) ) };
