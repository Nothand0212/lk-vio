#include "lk_vio/imu_preintegration.hpp"

namespace lk_vio
{
  // Lie Algebra Functions
  Eigen::Matrix3d NormalizeRotation( const Eigen::Matrix3d &R )
  {
    Eigen::JacobiSVD<Eigen::Matrix3d> svd( R, Eigen::ComputeFullU | Eigen::ComputeFullV );
    return svd.matrixU() * svd.matrixV().transpose();
  }

  Eigen::Matrix3d RightJacobianSO3( const float &x, const float &y, const float &z )
  {
    Eigen::Matrix3d I;
    I.setIdentity();
    const float     d2 = x * x + y * y + z * z;
    const float     d  = sqrt( d2 );
    Eigen::Vector3d v;
    v << x, y, z;
    Eigen::Matrix3d W = Sophus::SO3d::hat( v );
    if ( d < EPSILON )
    {
      return I;
    }
    else
    {
      return I - W * ( 1.0f - cos( d ) ) / d2 + W * W * ( d - sin( d ) ) / ( d2 * d );
    }
  }

  Eigen::Matrix3d RightJacobianSO3( const Eigen::Vector3d &v )
  {
    return RightJacobianSO3( v( 0 ), v( 1 ), v( 2 ) );
  }

  Eigen::Matrix3d InverseRightJacobianSO3( const float &x, const float &y, const float &z )
  {
    Eigen::Matrix3d I;
    I.setIdentity();
    const float     d2 = x * x + y * y + z * z;
    const float     d  = sqrt( d2 );
    Eigen::Vector3d v;
    v << x, y, z;
    Eigen::Matrix3d W = Sophus::SO3d::hat( v );

    if ( d < EPSILON )
    {
      return I;
    }
    else
    {
      return I + W / 2 + W * W * ( 1.0f / d2 - ( 1.0f + cos( d ) ) / ( 2.0f * d * sin( d ) ) );
    }
  }

  Eigen::Matrix3d InverseRightJacobianSO3( const Eigen::Vector3d &v )
  {
    return InverseRightJacobianSO3( v( 0 ), v( 1 ), v( 2 ) );
  }


  IMUPreintegrator::IMUPreintegrator( const IMUBias &b_, const IMUCalibration &calib )
  {
    Nga     = calib.cov;
    NgaWalk = calib.cov_walk;
    Initialize( b_ );
  }

  // Copy constructor
  IMUPreintegrator::IMUPreintegrator( IMUPreintegrator *pImuPre ) : dT( pImuPre->dT ), C( pImuPre->C ), Info( pImuPre->Info ), Nga( pImuPre->Nga ), NgaWalk( pImuPre->NgaWalk ), b( pImuPre->b ), dR( pImuPre->dR ), dV( pImuPre->dV ), dP( pImuPre->dP ), JRg( pImuPre->JRg ), JVg( pImuPre->JVg ), JVa( pImuPre->JVa ), JPg( pImuPre->JPg ), JPa( pImuPre->JPa ), avgA( pImuPre->avgA ), avgW( pImuPre->avgW ), bu( pImuPre->bu ), db( pImuPre->db ), mvMeasurements( pImuPre->mvMeasurements )
  {
  }

  void IMUPreintegrator::CopyFrom( IMUPreintegrator *pImuPre )
  {
    dT      = pImuPre->dT;
    C       = pImuPre->C;
    Info    = pImuPre->Info;
    Nga     = pImuPre->Nga;
    NgaWalk = pImuPre->NgaWalk;
    b.copyFrom( pImuPre->b );
    dR   = pImuPre->dR;
    dV   = pImuPre->dV;
    dP   = pImuPre->dP;
    JRg  = pImuPre->JRg;
    JVg  = pImuPre->JVg;
    JVa  = pImuPre->JVa;
    JPg  = pImuPre->JPg;
    JPa  = pImuPre->JPa;
    avgA = pImuPre->avgA;
    avgW = pImuPre->avgW;
    bu.copyFrom( pImuPre->bu );
    db             = pImuPre->db;
    mvMeasurements = pImuPre->mvMeasurements;
  }


  void IMUPreintegrator::Initialize( const IMUBias &b_ )
  {
    dR.setIdentity();
    dV.setZero();
    dP.setZero();
    JRg.setZero();
    JVg.setZero();
    JVa.setZero();
    JPg.setZero();
    JPa.setZero();
    C.setZero();
    Info.setZero();
    db.setZero();
    b  = b_;
    bu = b_;
    avgA.setZero();
    avgW.setZero();
    dT = 0.0f;
    mvMeasurements.clear();
  }

  void IMUPreintegrator::Reintegrate()
  {
    std::unique_lock<std::mutex>  lock( mMutex );
    const std::vector<integrable> aux = mvMeasurements;
    Initialize( bu );
    for ( size_t i = 0; i < aux.size(); i++ )
      IntegrateNewMeasurement( aux[ i ].a, aux[ i ].w, aux[ i ].t );
  }

  void IMUPreintegrator::IntegrateNewMeasurement( const Eigen::Vector3d &acceleration, const Eigen::Vector3d &angVel, const float &dt )
  {
    mvMeasurements.push_back( integrable( acceleration, angVel, dt ) );

    // Position is updated firstly, as it depends on previously computed velocity and rotation.
    // Velocity is updated secondly, as it depends on previously computed rotation.
    // Rotation is the last to be updated.

    //Matrices to compute covariance
    Eigen::Matrix<double, 9, 9> A;
    A.setIdentity();
    Eigen::Matrix<double, 9, 6> B;
    B.setZero();

    Eigen::Vector3d acc, accW;
    acc << acceleration( 0 ) - b.bax, acceleration( 1 ) - b.bay, acceleration( 2 ) - b.baz;
    accW << angVel( 0 ) - b.bwx, angVel( 1 ) - b.bwy, angVel( 2 ) - b.bwz;

    avgA = ( dT * avgA + dR * acc * dt ) / ( dT + dt );
    avgW = ( dT * avgW + accW * dt ) / ( dT + dt );

    // Update delta position dP and velocity dV (rely on no-updated delta rotation)
    dP = dP + dV * dt + 0.5f * dR * acc * dt * dt;
    dV = dV + dR * acc * dt;

    // Compute velocity and position parts of matrices A and B (rely on non-updated delta rotation)
    Eigen::Matrix<double, 3, 3> Wacc = Sophus::SO3d::hat( acc );

    A.block<3, 3>( 3, 0 ) = -dR * dt * Wacc;
    A.block<3, 3>( 6, 0 ) = -0.5f * dR * dt * dt * Wacc;
    A.block<3, 3>( 6, 3 ) = Eigen::DiagonalMatrix<double, 3>( dt, dt, dt );
    B.block<3, 3>( 3, 3 ) = dR * dt;
    B.block<3, 3>( 6, 3 ) = 0.5f * dR * dt * dt;


    // Update position and velocity jacobians wrt bias correction
    JPa = JPa + JVa * dt - 0.5f * dR * dt * dt;
    JPg = JPg + JVg * dt - 0.5f * dR * dt * dt * Wacc * JRg;
    JVa = JVa - dR * dt;
    JVg = JVg - dR * dt * Wacc * JRg;

    // Update delta rotation
    IntegratedRotation dRi( angVel, b, dt );
    dR = NormalizeRotation( dR * dRi.delta_rotation );

    // Compute rotation parts of matrices A and B
    A.block<3, 3>( 0, 0 ) = dRi.delta_rotation.transpose();
    B.block<3, 3>( 0, 0 ) = dRi.right_jacobian * dt;

    // Update covariance
    C.block<9, 9>( 0, 0 ) = A * C.block<9, 9>( 0, 0 ) * A.transpose() + B * Nga * B.transpose();
    C.block<6, 6>( 9, 9 ) += NgaWalk;

    // Update rotation jacobian wrt bias correction
    JRg = dRi.delta_rotation.transpose() * JRg - dRi.right_jacobian * dt;

    // Total integrated time
    dT += dt;
  }

  void IMUPreintegrator::MergePrevious( IMUPreintegrator *pPrev )
  {
    if ( pPrev == this )
      return;

    std::unique_lock<std::mutex> lock1( mMutex );
    std::unique_lock<std::mutex> lock2( pPrev->mMutex );
    IMUBias                      bav;
    bav.bwx = bu.bwx;
    bav.bwy = bu.bwy;
    bav.bwz = bu.bwz;
    bav.bax = bu.bax;
    bav.bay = bu.bay;
    bav.baz = bu.baz;

    const std::vector<integrable> aux1 = pPrev->mvMeasurements;
    const std::vector<integrable> aux2 = mvMeasurements;

    Initialize( bav );
    for ( size_t i = 0; i < aux1.size(); i++ )
      IntegrateNewMeasurement( aux1[ i ].a, aux1[ i ].w, aux1[ i ].t );
    for ( size_t i = 0; i < aux2.size(); i++ )
      IntegrateNewMeasurement( aux2[ i ].a, aux2[ i ].w, aux2[ i ].t );
  }

  void IMUPreintegrator::SetNewBias( const IMUBias &bu_ )
  {
    std::unique_lock<std::mutex> lock( mMutex );
    bu = bu_;

    db( 0 ) = bu_.bwx - b.bwx;
    db( 1 ) = bu_.bwy - b.bwy;
    db( 2 ) = bu_.bwz - b.bwz;
    db( 3 ) = bu_.bax - b.bax;
    db( 4 ) = bu_.bay - b.bay;
    db( 5 ) = bu_.baz - b.baz;
  }

  IMUBias IMUPreintegrator::GetDeltaBias( const IMUBias &b_ )
  {
    std::unique_lock<std::mutex> lock( mMutex );
    return IMUBias( b_.bax - b.bax, b_.bay - b.bay, b_.baz - b.baz, b_.bwx - b.bwx, b_.bwy - b.bwy, b_.bwz - b.bwz );
  }


  Eigen::Matrix3d IMUPreintegrator::GetDeltaRotation( const IMUBias &b_ )
  {
    std::unique_lock<std::mutex> lock( mMutex );
    Eigen::Vector3d              dbg;
    dbg << b_.bwx - b.bwx, b_.bwy - b.bwy, b_.bwz - b.bwz;
    return NormalizeRotation( dR * Sophus::SO3d::exp( JRg * dbg ).matrix() );
  }

  Eigen::Vector3d IMUPreintegrator::GetDeltaVelocity( const IMUBias &b_ )
  {
    std::unique_lock<std::mutex> lock( mMutex );
    Eigen::Vector3d              dbg, dba;
    dbg << b_.bwx - b.bwx, b_.bwy - b.bwy, b_.bwz - b.bwz;
    dba << b_.bax - b.bax, b_.bay - b.bay, b_.baz - b.baz;
    return dV + JVg * dbg + JVa * dba;
  }

  Eigen::Vector3d IMUPreintegrator::GetDeltaPosition( const IMUBias &b_ )
  {
    std::unique_lock<std::mutex> lock( mMutex );
    Eigen::Vector3d              dbg, dba;
    dbg << b_.bwx - b.bwx, b_.bwy - b.bwy, b_.bwz - b.bwz;
    dba << b_.bax - b.bax, b_.bay - b.bay, b_.baz - b.baz;
    return dP + JPg * dbg + JPa * dba;
  }

  Eigen::Matrix3d IMUPreintegrator::GetUpdatedDeltaRotation()
  {
    std::unique_lock<std::mutex> lock( mMutex );
    return NormalizeRotation( dR * Sophus::SO3d::exp( JRg * db.head( 3 ) ).matrix() );
  }

  Eigen::Vector3d IMUPreintegrator::GetUpdatedDeltaVelocity()
  {
    std::unique_lock<std::mutex> lock( mMutex );
    return dV + JVg * db.head( 3 ) + JVa * db.tail( 3 );
  }

  Eigen::Vector3d IMUPreintegrator::GetUpdatedDeltaPosition()
  {
    std::unique_lock<std::mutex> lock( mMutex );
    return dP + JPg * db.head( 3 ) + JPa * db.tail( 3 );
  }

  Eigen::Matrix3d IMUPreintegrator::GetOriginalDeltaRotation()
  {
    std::unique_lock<std::mutex> lock( mMutex );
    return dR;
  }

  Eigen::Vector3d IMUPreintegrator::GetOriginalDeltaVelocity()
  {
    std::unique_lock<std::mutex> lock( mMutex );
    return dV;
  }

  Eigen::Vector3d IMUPreintegrator::GetOriginalDeltaPosition()
  {
    std::unique_lock<std::mutex> lock( mMutex );
    return dP;
  }

  IMUBias IMUPreintegrator::GetOriginalBias()
  {
    std::unique_lock<std::mutex> lock( mMutex );
    return b;
  }

  IMUBias IMUPreintegrator::GetUpdatedBias()
  {
    std::unique_lock<std::mutex> lock( mMutex );
    return bu;
  }

  Eigen::Matrix<double, 6, 1> IMUPreintegrator::GetDeltaBias()
  {
    std::unique_lock<std::mutex> lock( mMutex );
    return db;
  }
}  // namespace lk_vio