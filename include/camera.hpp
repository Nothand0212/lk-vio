...
public:
  ...
  Camera( double fx, double fy, double cx, double cy, double baseline, const Sophus::SE3d &pose, const cv::Mat dist_coef )
  ...
  Camera( const Eigen::Matrix3d &K, const double &baseline, const Sophus::SE3d &pose, const cv::Mat &dist_coef )
  ...
  Eigen::Vector3d worldToCamera( const Eigen::Vector3d &point_world, const Sophus::SE3d &T_c_w );
  Eigen::Vector3d cameraToWorld( const Eigen::Vector3d &point_camera, const Sophus::SE3d &T_c_w );

  Eigen::Vector2d cameraToPixel( const Eigen::Vector3d &point_camera );
  Eigen::Vector3d pixelToCamera( const Eigen::Vector2d &point_pixel, const double &depth = 1 );

  Eigen::Vector3d pixelToWorld( const Eigen::Vector2d &point_pixel, const Sophus::SE3d &T_c_w, const double &depth = 1 );
