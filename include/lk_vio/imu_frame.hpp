...
namespace lk_vio
{
  ...
  public:
    ...
    Eigen::Vector3d a;  // linear acceleration in m/s^2
    Eigen::Vector3d w;  // angular velocity in rad/s
  ...
  public:
    ...
    Eigen::DiagonalMatrix<double, 6> cov, cov_walk;
    bool                             is_set;
  ...
  public:
    ...
    IntegratedRotation( const Eigen::Vector3d &angle_velocity, const IMUBias &imu_bias, const float &delta_time )
    {
      ...
      Eigen::Matrix3d skew_symmetric_matrix = Sophus::SO3d::hat( rotation_vector );
      if ( norm < EPSILON )
      {
        delta_rotation = Eigen::Matrix3d::Identity() + skew_symmetric_matrix;
        right_jacobian = Eigen::Matrix3d::Identity();
      }
      else
      {
        delta_rotation = Eigen::Matrix3d::Identity() + skew_symmetric_matrix * sin( norm ) / norm + skew_symmetric_matrix * skew_symmetric_matrix * ( 1 - cos( norm ) ) / norm_squared;
        right_jacobian = Eigen::Matrix3d::Identity() + skew_symmetric_matrix * ( 1 - cos( norm ) ) / norm_squared + skew_symmetric_matrix * skew_symmetric_matrix * ( norm - sin( norm ) ) / ( norm * norm_squared );
      }
    }
  };
